...
function getAdminClients() {
  try {
    ...
    console.error("[SERVERLESS MEDIA UPLOAD] Error initializing Firebase Admin:", err);
  ...
  try {
    ...
    if (!isSuperFounder && adminDb) {
      try {
        ...
        console.warn("[SERVERLESS MEDIA UPLOAD DB CHECK WARNING]", dbErr);
      }
    }
    ...
    if (!supabaseKey) {
      return res.status(500).json({
        success: false,
        error: "Clé de service Supabase manquante dans les variables d'environnement du serveur."
      });
    }
    ...
    if (uploadError) {
      console.error("[SERVERLESS MEDIA UPLOAD STORAGE ERROR]", uploadError);
      return res.status(500).json({
        success: false,
