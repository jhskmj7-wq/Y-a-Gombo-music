...
async function verifyIdTokenREST(idToken: string): Promise<{ uid: string; email: string } | null> {
  try {
    ...
    if (!response.ok) {
      console.error("[COVER UPLOAD SERVERLESS] REST Auth Verification failed, status:", response.status);
      return null;
    }
    ...
    console.error("[COVER UPLOAD SERVERLESS] REST Auth Exception:", err);
    return null;
  }
}
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    console.error("[COVER UPLOAD SERVERLESS FATAL ERROR]", err);
    return res.status(500).json({
      success: false,
      error: err?.message || "Erreur interne lors du téléversement de la bannière."
    });
  }
}
