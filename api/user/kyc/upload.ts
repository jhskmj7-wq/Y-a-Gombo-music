...
async function verifyIdTokenREST(idToken: string): Promise<{ uid: string; email: string } | null> {
  try {
    ...
    if (!response.ok) {
      console.error("[KYC UPLOAD SERVERLESS] REST Auth Verification failed, status:", response.status);
      return null;
    }
    ...
    console.error("[KYC UPLOAD SERVERLESS] REST Auth Exception:", err);
    return null;
  }
}
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (!supabaseKey) {
      return res.status(500).json({ success: false, error: "Configuration serveur manquante (clé service_role)." });
    }
    ...
    if (uploadError) {
      ...
      return res.status(500).json({ success: false, error: uploadError.message || "Échec du téléversement du document." });
    }
    ...
    console.error("[KYC UPLOAD SERVERLESS FATAL ERROR]", err);
