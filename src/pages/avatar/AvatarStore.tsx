...
export const AvatarStore: React.FC<AvatarStoreProps> = ({ currentUser, currentUserProfile, onBack }) => {
  ...
  return (
    <PageContainer className="p-0 bg-black text-white" id="avatar-store-page-root">
      {/* Fixed Android Top Header */}
      <header className="sticky top-0 z-40 bg-zinc-950/95 backdrop-blur-md border-b border-zinc-800 px-4 py-3.5 flex items-center justify-between w-full max-w-full box-border" id="avatar-store-header">
        <button 
          onClick={onBack}
      ...
      <ScrollContainer className="flex-1 p-4 space-y-5" id="avatar-store-scroll-container">
        ...
        {loading ? (
          ...
          <div className="py-20 text-center text-zinc-500 font-mono text-xs space-y-2" id="avatar-store-empty">
          ...
          <div className="grid grid-cols-2 gap-3.5 w-full max-w-full box-border" id="avatar-store-items-grid">
            {filteredItems.map((item) => {
              ...
              return (
                <div 
                  key={item.id} 
                  className="bg-zinc-900/90 border border-zinc-800/80 rounded-2xl p-3 flex flex-col justify-between space-y-3 w-full max-w-full box-border shadow-md"
                ...
                >
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-[10px] text-zinc-400 line-clamp-1">{item.description || 'Article exclusif Afrigombo'}</p>
                  </div>
