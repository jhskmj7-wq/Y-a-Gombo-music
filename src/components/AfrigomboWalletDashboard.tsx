...
export default function AfrigomboWalletDashboard({ 
  ...
  const [gawaHistory, setGawaHistory] = useState<any[]>([]);
  ...
  useEffect(() => {
    ...
    const unsubProfile = onSnapshot(doc(db, "users", uid), (snap) => {
      if (snap.exists()) {
        ...
        setWalletPinSettings({
          pinHash: uData.walletSecurity?.pinHash || "",
          pinEnabled: uData.paymentSettings?.pinEnabled ?? !!uData.walletSecurity?.pinHash,
          pinLength: uData.walletSecurity?.pinLength || 6,
        });
      }
      setLoading(false);
    });
    ...
    const unsubGawaHistory = GawaEngineService.subscribeUserGawaHistory(uid, (history) => {
      setGawaHistory(history);
    });
    const unsubMaintenance = typeof useMaintenance === 'function' ? (useMaintenance as any).subscribe?.(() => {}) : undefined;
  ...
  const handleWithdrawRequest = async (e: React.FormEvent) => {
    ...
    const ok = await requireWalletAuthentication("DEMANDE_DE_RETRAIT");
    if (ok) {
        executeFn();
    }
  };
  ...
  const handleTransferSubmit = async (e: React.FormEvent) => {
    ...
    // Use centralized authentication
    const ok = await requireWalletAuthentication("TRANSFERT_P2P");
    if (ok) {
        executeFn();
    }
  };
  ...
  if (showWalletSettings) {
                  ...
                  {!walletPinSettings.pinHash ? (
                    ...
                    <>
                      <button
                        ...
                        onClick={async () => {
                          playSound("click");
                          const ok = await changeWalletPin();
                          if (ok) {
    ...
    if (!isAuthorized) {
      return (
        <AndroidPageLayout scrollable={false} className="bg-afri-bg flex items-center justify-center">
            <div className="flex flex-col items-center gap-4">
                <Loader2 className="w-10 h-10 text-afri-gold animate-spin" />
