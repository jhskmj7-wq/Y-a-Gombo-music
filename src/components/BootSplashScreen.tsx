...
export default function BootSplashScreen({ onComplete }: BootSplashScreenProps) {
  ...
  return (
    <div className="fixed inset-0 z-[99999] bg-[#0A0A10] text-afri-text flex flex-col items-center justify-center p-6 select-none font-mono">
      <div className="max-w-sm w-full bg-afri-bg/90 border border-[#D4AF37]/30 rounded-3xl p-8 backdrop-blur-xl shadow-2xl flex flex-col items-center text-center space-y-6">
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs text-afri-text-sec uppercase tracking-wider font-sans font-medium">
            Le Temple du Gombo Musical
          </p>
        </div>

        {/* Smooth Gold Progress Bar */}
        <div className="w-full bg-afri-bg-sec h-2 rounded-full overflow-hidden border border-afri-border relative my-2">
          <div 
        ...
        <div className="pt-2">
          <button
            ...
            className="text-[10px] text-zinc-600 hover:text-afri-text-sec underline uppercase tracking-wider cursor-pointer transition"
          >
            Nettoyer les caches
          </button>
        </div>
