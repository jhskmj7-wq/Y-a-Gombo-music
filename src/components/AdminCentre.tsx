...
export default function AdminCentre({ theme, toggleTheme }: AdminCentreProps) {
  ...
  useEffect(() => {
    ...
    if (!client) {
      setPosts([]);
      setPostsDebugError("Supabase n’est pas configuré pour charger les Réels.");
      return () => {
        isActive = false;
      };
    }
    ...
    const loadUserReels = async () => {
      ...
      if (error) {
        console.warn("⚠️ [SUPABASE_REELS] Impossible de charger les Réels:", error.message);
        setPosts([]);
        setPostsDebugError(`Supabase — ${error.message}`);
        return;
      }
