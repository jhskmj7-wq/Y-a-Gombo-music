...
export default function AdminCentre({ theme, toggleTheme }: AdminCentreProps) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const unsubscribePosts = onSnapshot(qPosts, (snapshot) => {
        ...
        console.warn("🔐 Posts sync limited or offline (données existantes conservées):", error.code, error.message);
        setPostsDebugError(`${error.code || "?"} — ${error.message}`);
        // Les posts existants dans le state sont préservés intacts (aucun appel setPosts([])).
        // Le listener onSnapshot reconnecte automatiquement le stream dès retour du réseau.
      });
  ...
  return (
    <div className={`flex h-full h-[100dvh] w-full max-w-full box-border bg-afri-bg text-afri-text font-sans antialiased uppercase-none`}>
            ...
            {activeMenu !== "user_terrain" && activeMenu !== "nearby" && (
              <motion.div
                ...
                className={`flex-1 min-h-0 h-full w-full ${
                  ["user_settings", "user_notifications", "user_messages", "user_reels", "user_wallet", "super_admin"].includes(activeMenu) || (activeMenu === "user_heritage" && isHeritageSubPanelActive)
      ...
      {/* FLOATING MESSAGING BUBBLE (Arbre à Palabres) */}
      {activeMenu !== "user_messages" && (
        <Suspense fallback={null}>
