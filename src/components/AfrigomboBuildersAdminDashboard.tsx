    ...
    <div className="space-y-6 text-left pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-afri-bg-sec border border-afri-border rounded-3xl p-6">
          <h3 className="text-xs font-black text-afri-text uppercase tracking-wider mb-6 flex items-center gap-2">
            <Trophy className="w-4 h-4 text-amber-500" />
            Top Bâtisseurs
          </h3>
          <div className="space-y-4">
            {stats.topContributors.length === 0 ? (
              <p className="text-xs text-afri-text-sec font-mono text-center py-6">Aucun contributeur.</p>
            ) : (
              stats.topContributors.map((c, idx) => (
                <div key={c.userId} className="flex items-center justify-between bg-afri-bg p-3 rounded-xl border border-afri-border">
                  <div className="flex items-center gap-3">
                    <span className="text-xs font-black text-[#D4AF37]">#{idx + 1}</span>
                    <span className="text-xs font-bold text-afri-text">{c.userId.substring(0,8)}...</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="text-xs font-mono text-afri-text bg-afri-bg px-2 py-1 rounded-md">{c.amount.toLocaleString()} FCFA</span>
                  </div>
                </div>
              ))
            )}
          ...
          <div className="space-y-4">
            {Object.entries(stats.badgesDistributed).map(([badge, count]) => (
              <div key={badge} className="flex items-center justify-between">
                <span className="text-xs font-bold text-afri-text">{badge}</span>
                <span className="text-[10px] font-mono font-black text-afri-text bg-afri-bg px-3 py-1 rounded-full">{count as number}</span>
              </div>
            ))}
          </div>
        </div>

        <div className="bg-afri-bg-sec border border-afri-border rounded-3xl p-6 flex flex-col">
          <h3 className="text-xs font-black text-afri-text uppercase tracking-wider mb-6 flex items-center gap-2">
            <Flame className="w-4 h-4 text-red-500" />
            Gestion des Défis
          </h3>
          
          <form onSubmit={handleCreateChallenge} className="mb-6 space-y-3 bg-afri-bg p-4 rounded-xl border border-afri-border">
            <input 
              type="text" 
          ...
          {challenges.length === 0 ? (
            <p className="text-xs text-afri-text-sec font-mono text-center py-6">Aucun défi configuré.</p>
          ) : (
            <div className="space-y-4 max-h-[300px] overflow-y-auto pr-2">
              {challenges.map(c => {
                const p = Math.min(100, Math.round((c.current / c.target) * 100));
                return (
                  <div key={c.id} className="bg-afri-bg border border-afri-border p-4 rounded-2xl relative group">
                    <button 
                      onClick={() => handleCloseChallenge(c.id)}
                      className="absolute top-2 right-2 p-1.5 bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-afri-text rounded-md opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer"
                    >
                      <Trash2 className="w-3 h-3" />
                    </button>
                    <div className="flex justify-between items-center mb-2 pr-6">
                      <h4 className="text-sm font-bold text-afri-text">{c.title}</h4>
