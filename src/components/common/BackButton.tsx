...
export function BackButton({ onClick, className = "", label }: BackButtonProps) {
  return (
    <button
      onClick={onClick}
      className={`min-h-[48px] flex items-center justify-center rounded-xl bg-afri-bg-sec border border-afri-border hover:bg-afri-bg-sec/80 transition-colors cursor-pointer ${label ? 'px-4 gap-2' : 'w-12'} ${className}`}
