...
export default function PWAHandler() {
  ...
  return (
    <>
      ...
      {needRefresh && (
        <div className="fixed bottom-20 right-4 left-4 sm:left-auto sm:max-w-md z-[9999] bg-afri-bg-sec backdrop-blur-xl border-2 border-[#D4AF37] p-4 sm:p-5 rounded-2xl shadow-[0_10px_40px_rgba(212,175,55,0.4)] text-afri-text flex flex-col gap-3.5 animate-fadeIn">
          <div className="flex items-center justify-between">
            ...
            <button 
              ...
              className="text-afri-text-sec hover:text-afri-text p-1 rounded-lg transition-colors cursor-pointer"
              title="Masquer"
            >
              <X className="w-5 h-5" />
            </button>
          ...
          <div className="flex items-center justify-end gap-2 pt-1">
            <button
              onClick={handleDismiss}
              className="px-3 py-2 text-xs font-bold text-afri-text-sec hover:text-afri-text transition-colors cursor-pointer"
            >
              Plus tard
            </button>
            <button
              ...
              disabled={isUpdating}
              className="px-5 py-2.5 bg-gradient-to-r from-[#D4AF37] to-amber-500 text-zinc-950 font-black text-xs rounded-xl hover:from-yellow-400 hover:to-amber-400 transition-all shadow-lg active:scale-95 cursor-pointer uppercase tracking-wider flex items-center gap-2"
            >
      ...
      {showInstallBanner && deferredPrompt && !installed && !needRefresh && (
        <div className="fixed bottom-4 left-4 right-4 md:left-auto md:right-4 md:max-w-md z-[9998] bg-afri-bg-sec backdrop-blur-lg border border-[#D4AF37]/40 p-4 rounded-2xl shadow-2xl text-afri-text flex items-center justify-between gap-3 animate-fadeIn">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="font-bold text-sm text-afri-text flex items-center gap-1.5">
                <span>Installer AfriGombo</span>
                <span className="text-[10px] px-1.5 py-0.5 bg-[#D4AF37]/20 text-[#D4AF37] rounded font-mono">PWA</span>
              </h4>
              <p className="text-xs text-afri-text-sec">Accès direct et rapide depuis l'écran d'accueil.</p>
            </div>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <button
              onClick={handleInstall}
              className="px-3.5 py-2 bg-[#D4AF37] text-zinc-950 font-black text-xs rounded-xl hover:bg-yellow-400 transition-all shadow-md cursor-pointer uppercase tracking-wider"
            >
              Installer
            </button>
            <button
              onClick={() => setShowInstallBanner(false)}
              className="p-1.5 text-afri-text-sec hover:text-afri-text rounded-lg cursor-pointer"
              title="Fermer"
            >
              <X className="w-4 h-4" />
            </button>
