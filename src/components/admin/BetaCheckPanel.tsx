...
export default function BetaCheckPanel() {
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 animate-fadeIn font-sans max-w-5xl mx-auto">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-afri-bg-sec border border-afri-border p-5 rounded-2xl">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-zinc-900/80 border border-emerald-500/30 p-4 rounded-xl text-center space-y-1">
          <p className="text-xs text-afri-text-sec font-bold uppercase">🟢 Fonctionnels</p>
        ...
        <div className="bg-zinc-900/80 border border-amber-500/30 p-4 rounded-xl text-center space-y-1">
          <p className="text-xs text-afri-text-sec font-bold uppercase">🟡 À Vérifier</p>
        ...
        <div className="bg-zinc-900/80 border border-rose-500/30 p-4 rounded-xl text-center space-y-1">
          <p className="text-xs text-afri-text-sec font-bold uppercase">🔴 Anomalies</p>
          <p className="text-2xl font-black text-rose-400">{errCount}</p>
        </div>
      </div>

      {/* Tabs */}
      <div className="flex border-b border-afri-border gap-4">
        <button
          onClick={() => setActiveTab("audit")}
          className={`pb-3 text-xs font-bold uppercase tracking-wider border-b-2 transition ${
            activeTab === "audit"
              ? "border-[#D4AF37] text-[#D4AF37]"
              : "border-transparent text-afri-text-sec hover:text-afri-text"
          }`}
        >
        ...
        <div className="grid gap-3">
          {results.map((res, index) => (
            <div
              ...
              className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-afri-bg-sec rounded-2xl border border-afri-border gap-3 hover:border-afri-border transition"
            >
              <div className="space-y-0.5">
                <span className="text-[10px] font-mono uppercase tracking-widest text-[#D4AF37] font-bold">
                  {res.module}
                </span>
                <h3 className="font-bold text-afri-text text-sm">{res.service}</h3>
                <p className="text-xs text-afri-text-sec">{res.message}</p>
              </div>
      ...
      {activeTab === "bug_reports" && (
        <div className="space-y-4">
          <div className="flex justify-between items-center text-xs text-afri-text-sec">
            <span>
          ...
          {loadingBugs ? (
            <div className="p-8 text-center text-afri-text-sec font-mono text-xs">
              Chargement du journal d'erreurs Firestore...
            </div>
          ) : bugReports.length === 0 ? (
            <div className="p-8 text-center bg-zinc-900/60 rounded-2xl border border-afri-border text-afri-text-sec text-xs">
            ...
            <div className="space-y-3">
              {bugReports.map((report) => (
                <div
                  key={report.id}
                  className="bg-afri-bg-sec border border-afri-border rounded-2xl p-4 space-y-2 text-xs font-mono"
                >
                  ...
                  <p className="text-afri-text font-sans text-xs font-semibold">{report.message}</p>

                  {report.stack && (
                    <pre className="p-2 bg-afri-bg/60 rounded-lg text-[10px] text-afri-text-sec overflow-x-auto max-h-24">
                      {report.stack}
                    </pre>
                  )}

                  <div className="flex flex-wrap gap-x-4 gap-y-1 text-[10px] text-afri-text-muted font-sans pt-1">
