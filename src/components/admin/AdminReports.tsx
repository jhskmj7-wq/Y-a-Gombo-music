    ...
    <div className="space-y-8 text-left pb-24 animate-fadeIn">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          {alerts.length === 0 ? (
            <div className="p-6 text-center bg-black/40 border border-zinc-900 rounded-2xl text-zinc-500 text-xs font-mono">
            ...
            alerts.map((alert) => (
              <div
                ...
                className={`p-5 rounded-2xl border ${
                  alert.severity === "high" ? "border-red-500/30 bg-red-500/5" : "border-[#D4AF37]/20 bg-[#070707]"
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <span className="font-sans font-black text-sm text-white block">
                      Alerte - {alert.userArtisticName || "Artiste Gombo"}
                    </span>
                    <span className="text-xs text-zinc-400 leading-relaxed block mt-1">{alert.reason}</span>
                ...
                <div className="flex gap-2 w-full sm:w-auto self-end sm:self-center">
                  {onDismissAlert && (
                    <button
                      ...
                      className="px-4 py-2 bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 text-zinc-300 font-mono font-bold text-[10px] uppercase rounded-xl transition-all flex items-center gap-1.5 cursor-pointer"
                    >
        ...
        <div className="space-y-3">
          {flaggedPosts.length === 0 ? (
            ...
            flaggedPosts.map((post) => (
              <div
                key={post.id}
                className="p-5 bg-gradient-to-br from-[#070707] to-black border border-red-500/20 rounded-2xl space-y-4"
              >
                <div className="flex justify-between items-start border-b border-white/5 pb-3">
                  <div>
                    <span className="text-xs text-zinc-400 block font-bold font-mono">Auteur</span>
                    <strong className="text-sm font-sans font-black text-white">{post.authorArtisticName || post.authorName}</strong>
                  </div>
                ...
                <p className="text-xs text-zinc-300 leading-relaxed bg-black/60 p-4 border border-zinc-900 rounded-xl font-sans">
                  {post.content}
                </p>
                ...
                <div className="flex gap-2">
                  {onUnflagPost && (
                    <button
                      ...
                      className="flex-1 py-2 rounded-xl bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 text-zinc-300 font-mono font-bold text-[10px] uppercase flex items-center justify-center gap-1.5 transition-all cursor-pointer"
                    >
