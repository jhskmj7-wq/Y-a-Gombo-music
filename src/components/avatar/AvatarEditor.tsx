...
export default function AvatarEditor({ onClose }: AvatarEditorProps) {
  ...
  if (loading) {
    return (
      <div className="fixed inset-0 z-[110] flex flex-col items-center justify-center bg-afri-bg/80 backdrop-blur-md text-afri-text font-mono text-xs gap-3">
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-3 sm:p-4 bg-afri-bg/80 backdrop-blur-md animate-fadeIn text-left font-sans">
      <div className="w-full max-w-5xl h-[88vh] bg-afri-bg-sec border border-afri-border rounded-3xl overflow-hidden flex flex-col md:flex-row shadow-2xl relative">
        
        {/* Left Panel: Avatar Preview & Profile Options */}
        <div className="w-full md:w-1/3 bg-afri-bg border-r border-afri-border p-6 flex flex-col items-center justify-between relative">
          <div className="w-full flex items-center justify-between mb-4">
            <h2 className="text-lg font-black uppercase text-afri-text tracking-wider font-sans">
              MON AVATAR GOMBO
            </h2>
          ...
          <div className="flex-1 flex items-center justify-center my-4 w-full">
            <AvatarRenderer config={config} size={220} storeItems={storeItems} className="shadow-2xl rounded-3xl overflow-hidden border border-afri-border" />
          </div>
          
          <div className="w-full space-y-3">
            <label className="flex items-center gap-3 p-3.5 bg-afri-bg-sec rounded-2xl border border-afri-border cursor-pointer hover:border-[#D4AF37]/50 transition">
              <input 
                type="checkbox" 
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[10px] text-afri-text-sec">Remplacera votre photo Google publiquement</span>
              </div>
            </label>
        ...
        <div className="flex-1 flex flex-col h-full bg-zinc-900/40">
          <div className="flex items-center justify-between p-4 border-b border-afri-border">
            <span className="text-xs text-afri-text-sec font-mono">Personnalisation des pièces équipées</span>
            <div className="flex items-center gap-2">
              <button 
                onClick={onClose}
                className="px-4 py-2 rounded-xl text-xs font-bold text-afri-text-sec hover:text-afri-text transition cursor-pointer"
              >
                Annuler
              </button>
              <button 
          ...
          {/* Categories Tabs */}
          <div className="flex overflow-x-auto p-3.5 gap-2 border-b border-afri-border scrollbar-none bg-zinc-950/40">
          ...
          <div className="flex-1 overflow-y-auto p-6 scrollbar-none">
            {activeTab === 'couleur_peau' ? (
              <div className="space-y-4">
                <p className="text-xs text-afri-text-sec font-mono">Sélectionnez la teinte de peau de votre Avatar :</p>
                    ...
                    <button
                      ...
                      className={`w-12 h-12 rounded-2xl border-4 transition cursor-pointer ${
                        config.skinColor === color 
                          ...
                          : "border-afri-border hover:scale-105"
                      }`}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                {itemsInActiveTab.length === 0 ? (
                  <div className="col-span-full py-16 bg-afri-bg border border-afri-border rounded-3xl text-center space-y-3">
                  ...
                  itemsInActiveTab.map(item => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-[10px] font-bold text-center text-afri-text line-clamp-1 uppercase">
                          {item.name}
                        </span>
