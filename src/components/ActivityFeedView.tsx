...
export default function ActivityFeedView({ currentUserProfile, onNavigateView }: ActivityFeedViewProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      {/* Banner */}
      <div className="relative overflow-hidden bg-gradient-to-r from-orange-400 to-orange-600 text-afri-text rounded-3xl p-6 sm:p-8 shadow-xl mb-8 border border-orange-400/10">
        <div className="absolute right-0 top-0 translate-x-1/4 -translate-y-1/4 opacity-10 blur-xl">
          <Zap className="w-96 h-96" />
        </div>
        
        <div className="relative z-10">
          <span className="text-[10px] font-black tracking-widest bg-white/20 text-afri-text px-3 py-1 rounded-full uppercase mb-3 inline-block">
            Showbiz CI Live
          </span>
          <h1 className="text-3xl font-black tracking-tight uppercase flex items-center gap-2">
            <Rss className="w-8 h-8 text-afri-text animate-pulse" />
            L’Actu Du Showbiz
          </h1>
      ...
      <div className="flex flex-wrap gap-2 mb-6 text-xs font-bold uppercase tracking-tight">
        <button
          ...
          className={`px-4 py-2.5 rounded-xl border transition-all ${
            activeFilter === "all"
              ? "bg-afri-bg-sec text-afri-text border-[#7C3AED] shadow-sm"
        ...
        <button
          ...
          className={`px-4 py-2.5 rounded-xl border transition-all flex items-center gap-1.5 ${
            activeFilter === "talent"
              ? "bg-emerald-600 text-afri-text border-emerald-600 shadow-sm"
      ...
      {loading ? (
        <div className="py-20 text-center text-afri-text-sec dark:text-afri-text-sec font-bold text-sm">
            ...
            {filtered.map((item, index) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Main feed list card */}
                  <div className="bg-white dark:bg-afri-bg-sec border border-gray-150 dark:border-gray-800/80 rounded-2xl p-5 hover:border-orange-400 dark:hover:border-orange-500/50 shadow-sm transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="space-y-0.5">
                        <div className="flex items-center flex-wrap gap-2">
                          <span className="text-xs font-black uppercase tracking-wider text-gray-450 dark:text-afri-text-sec">
