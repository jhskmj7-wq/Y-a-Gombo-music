...
export default function AfrigomboHelpCenter({ onClose }: HelpCenterProps) {
  const sections = [
    ...
    { title: "Guides d'utilisation", icon: FileText, color: "text-zinc-400", reason: "Guides d'utilisation" },
    ...
    <div className="flex flex-col text-left animate-fadeIn w-full">
      <div className="max-w-xl mx-auto w-full flex-1 flex flex-col space-y-6 pt-4">
        <div className="flex justify-between items-center border-b border-zinc-900 pb-4">
          ...
          <button 
            onClick={onClose}
            className="px-4 py-2 bg-zinc-950 border border-zinc-850 hover:border-[#D4AF37] rounded-xl text-xs font-bold text-zinc-400 hover:text-white cursor-pointer transition-all"
          >
            Fermer ✕
          </button>
        </div>
        ...
        <div className="flex-1 space-y-6 pb-20">
          <div className="p-5 rounded-2xl bg-zinc-950 border border-zinc-900 text-zinc-400 text-xs leading-relaxed space-y-3">
            <p>
              <strong className="text-white">Le Support Officiel AFRIGOMBO</strong> est le seul canal autorisé pour obtenir une assistance technique, signaler un problème ou poser une question.
            </p>
            <p>
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div className="space-y-1">
                <h3 className="font-black text-white uppercase text-sm">WhatsApp Officiel AFRIGOMBO</h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {sections.map((section, index) => (
              <button 
                ...
                className="flex items-center gap-4 p-4 rounded-xl bg-zinc-950 border border-zinc-900 hover:border-[#D4AF37]/50 transition-all text-left cursor-pointer"
              >
                <div className={`p-2 bg-zinc-900 rounded-lg ${section.color}`}>
                  <section.icon className="w-5 h-5" />
                </div>
                <span className="text-xs font-bold text-white uppercase">{section.title}</span>
              </button>
            ))}
          </div>
        </div>
